...
function Form( { children, handleSubmit, button }) {
    ...
    return (
        <form className="form" noValidate onSubmit={handleSubmit} action="submit">
            ...
            <p className="form__text">{location.pathname === "/signup" ? 'Уже зарегистрированы?' : 'Ещё не зарегистрированы?'} <a className="form__link anchor-hover" href="/signin">{location.pathname === "/signup" ? 'Войти' : 'Регистрация'}</a></p>
        </form>
    );
  }
