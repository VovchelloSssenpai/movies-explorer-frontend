...
function SearchForm( { setSearchTerm, handleSearchSubmit, searchTerm, setIsShortFilm, isShortFilm } ) {
...
const handleSubmit = (e) => {
    e.preventDefault();
    !searchTerm.length > 0 ? setMovieError('Enter a key word') :  handleSearchSubmit(); 
}
...
useEffect(() => {
  ...
  if (savedSearchTerm) {
    ...
    return (
        ...
        <FilterCheckbox setIsShortFilm = {setIsShortFilm} isShortFilm={isShortFilm} ></FilterCheckbox>        
      </form>
    );
  }
